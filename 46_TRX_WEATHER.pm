...
sub TRX_WEATHER_Initialize($) {
    ...
    $hash->{AttrList} = "IODev ignore:1,0 do_not_notify:1,0 rain_offset " . $readingFnAttributes;

}

#####################################
sub TRX_WEATHER_Define($$) {
    my ( $hash, $def ) = @_;
    my @a = split( "[ \t][ \t]*", $def );
    ...
    if ( ( $code =~ /^CM160/ ) || ( $code =~ /^CM180/ ) || ( $code =~ /^REVOLT/ ) ) {
        return "wrong syntax: define <name> TRX_WEATHER code [scale_current scale_total add_total]"
            if ( int(@a) != 3 && int(@a) != 6 );
...
sub TRX_WEATHER_temperature {
    ...
    my $temp
        = ( ( ( $bytes->[$off] & 0x80 ) ? -1 : 1 ) * ( ( $bytes->[$off] & 0x7f ) * 256 + $bytes->[ $off + 1 ] ) ) / 10;
...
sub TRX_WEATHER_common_therm {
    ...
    if ($TRX_HEX_debug) {
        my $hexline = "";
        for ( my $i = 0; $i < @$bytes; $i++ ) { $hexline .= sprintf( "%02x", $bytes->[$i] ); }
...
sub TRX_WEATHER_battery {
    ...
    push @$res,
        ...
        device  => $dev,
        type    => 'batteryPercent',
        current => sprintf( "%d0%", $battery_level ),
        };
...
sub TRX_WEATHER_common_rain {
    ...
    my %devname = (    # HEXSTRING => "NAME"
        ...
        0x08 => "DAVIS_RAIN",          # DAVIS
        0x09 => "TFA20323301_RAIN",    # TFA 30.3233.01
    );
    ...
    if ( $dev_type ne "TX5_RAIN" and $dev_type ne 'TFA20323301_RAIN' and $dev_type ne 'DAVIS_RAIN' ) {
...
sub TRX_WEATHER_common_energy {
    ...
    my $energy_current = ( $bytes->[6] * 256 * 256 * 256 + $bytes->[7] * 256 * 256 + $bytes->[8] * 256 + $bytes->[9] );
...
sub TRX_WEATHER_Parse($$) {
    ...
    foreach $i (@res) {
        ...
        if ( $i->{type} eq "temp" ) {
            Log3 $name, 5,
                "TRX_WEATHER: name=$name device=$device_name Temperatur " . $i->{current} . " " . $i->{units};
            $val .= "T: " . $i->{current} . " ";
            ...
            Log3 $name, 5,
                "TRX_WEATHER: name=$name device=$device_name Temperature-bbq " . $i->{current} . " " . $i->{units};
            $val .= "TB: " . $i->{current} . " ";
            ...
            Log3 $name, 5,
                "TRX_WEATHER: name=$name device=$device_name Temperatur-food " . $i->{current} . " " . $i->{units};
            $val .= "TF: " . $i->{current} . " ";
            ...
            Log3 $name, 5,
                "TRX_WEATHER: name=$name device=$device_name Luftfeuchtigkeit " . $i->{current} . $i->{units};
            $val .= "H: " . $i->{current} . " ";
        ...
        elsif ( $i->{type} eq "batteryState" ) {
            Log3 $name, 5, "TRX_WEATHER: name=$name device=$device_name BatteryState " . $i->{current};
            readingsBulkUpdate( $def, "batteryState", $i->{current} );
        }
        elsif ( $i->{type} eq "batteryPercent" ) {
            Log3 $name, 5, "TRX_WEATHER: name=$name device=$device_name BatteryPct " . $i->{current};
            readingsBulkUpdate( $def, "batteryPercent", $i->{current} );
        }
        elsif ( $i->{type} eq "pressure" ) {
            Log3 $name, 5,
                  "TRX_WEATHER: name=$name device=$device_name Luftdruck "
                . $i->{current} . " "
                . $i->{units}
                . " Vorhersage="
                . $i->{forecast};
            ...
            Log3 $name, 5,
                  "TRX_WEATHER: name=$name device=$device_name wind_speed "
                . $i->{current}
                . " wind_avspeed "
                . $i->{average};
            $val .= "W: " . $i->{current} . " ";
            ...
            my $tr = $i->{current} + AttrNum( $name, 'rain_offset', 0 );

            Log3 $name, 5, "TRX_WEATHER: name=$name device=$device_name train " . $tr;
            $val .= "TR: " . $tr . " ";

            $sensor = "rain_total";
            readingsBulkUpdate( $def, $sensor, $tr );
        }
            ...
            Log3 $name, 5,
                "TRX_WEATHER: name=$name device=$device_name uv_val " . $i->{current} . " uv_risk " . $i->{risk};
            $val .= "UV: " . $i->{current} . " ";
            $val .= "UVR: " . $i->{risk} . " ";
            ...
            if ( defined( $def->{scale_current} ) ) {
                $energy_current = $energy_current * $def->{scale_current};
                Log3 $name, 5,
                    "TRX_WEATHER: name=$name device=$device_name energy_ch1 scale_current=" . $def->{scale_current};
            }
            ...
            if ( defined( $def->{scale_current} ) ) {
                $energy_current = $energy_current * $def->{scale_current};
                Log3 $name, 5,
                    "TRX_WEATHER: name=$name device=$device_name energy_ch2 scale_current=" . $def->{scale_current};
            }
            ...
            if ( defined( $def->{scale_current} ) ) {
                $energy_current = $energy_current * $def->{scale_current};
                Log3 $name, 5,
                    "TRX_WEATHER: name=$name device=$device_name energy_ch3 scale_current=" . $def->{scale_current};
            }
            ...
            if ( defined( $def->{scale_total} ) && defined( $def->{add_total} ) ) {
                ...
                Log3 $name, 5,
                    "TRX_WEATHER: name=$name device=$device_name energy_total scale_total=" . $def->{scale_total};
            }
            ...
            Log3 $name, 1,
                "TRX_WEATHER: name=$name device=$device_name UNKNOWN Type: " . $i->{type} . " Value: " . $i->{current};
        }
    }
...
<ul>
  ...
  <ul>
    ...
    <li>rain_offset: "rain_offset" is added to "rain_total"</li>
  </ul>
  <br>
</ul>
