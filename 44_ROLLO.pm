########################################################################################
# $Id: 44_ROLLO.pm 18436 2019-01-27 20:46:04Z KernSani $ #
...
#Log3 $name,1, "ROLLO ($name) Readings position and desired_position aren't used anymore. Execute \"deletereading $name position\" and \"deletereading $name desired_position\" to remove them";
    ...
    #### Stop when in motion and get new drive command https://forum.fhem.de/index.php/topic,95832.msg1054155.html#msg1054155
    if ( AttrVal( $name, 'rl_stopOnCmdWhileDrive', '' ) eq '1' ) {
        if ( ( ( $cmd eq 'open' ) || ( $cmd eq 'closed' ) ) && ( ReadingsVal( $name, 'state', '' ) =~ /drive/ ) ) {

            #Log3 $name, 3, "Stop drive cause of an additional drive command while device is in motion!";
            $cmd = "stop";
        }
    }
    ...
    if ( $arg ne "" && $arg >= 0 && $arg <= 100 ) {
        ...
        if ( $cmd eq "down" || $cmd eq "up" ) {
        	my $step = AttrVal($name, 'rl_stepUpDown', 10);
            # Recalculate the desired pct
            my $posin = ReadingsVal( $name, "pct", 0 );
            $posin = 100 - $posin if ( $typ eq "HomeKit" );
            $desiredPos = int( ( $posin - $step ) / 10 + 0.5 ) * 10;
            $desiredPos = int( ( $posin + $step ) / 10 + 0.5 ) * 10 if $cmd eq "down";
...
<div>
	...
	<h4>Attributes</h4>
			<ul>
				...
				<li><a name="rl_stepUpDown">rl_stepUpDown</a> <code>attr &lt;Rollo-Device&gt; rl_stepUpDown [number]</code>
					<br />Defines the step (in pct) for up and down Commands (default 10)<br></li>
				<li><a name="rl_stopOnCmdWhileDrive">rl_stopOnCmdWhileDrive</a> <code>attr &lt;Rollo-Device&gt; rl_stopOnCmdWhileDrive [0|1]</code>
					<br />If the attribute is set, the device stops (instead of changing direction) if an 'open' or 'closed' command is executed</li>
	...
	<ul>
	 ...
	 <h4>Set</h4>
			...
			<ul>
				...
				<li><a name="rl_stepUpDown">rl_stepUpDown</a> <code>attr &lt;Rollo-Device&gt; rl_stepUpDown [number]</code>
					<br />Gibt die Schrittweite (in pct) für die up und down Kommandos an (default 10)<br></li>
				<li><a name="rl_stopOnCmdWhileDrive">rl_stopOnCmdWhileDrive</a> <code>attr &lt;Rollo-Device&gt; rl_stopOnCmdWhileDrive [0|1]</code>
					<br />Ist das Attribut gesetzt, stoppt der Rollo wenn während der Fahrt 'open' oder 'closed' gesetzt wird (anstatt die Richtung zu wechseln)</li>
				<li><a name="disable">disable</a><code>attr &lt;Rollo-Device&gt; disable [0|1]</code>
